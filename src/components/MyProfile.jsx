...
export default function MyProfile({ currentUser }) {
  ...
  const user = users.find(u => u.name === currentUser?.name) || users[0];
    ...
    <div className="page">
      ...
      <div className="grid-3">
        <div className="metric-card">
          ...
          <div className="metric-sub">{user.videos >= 50 ? '✓ Target Met' : `Need ${50 - user.videos} more`}</div>
        </div>
        <div className="metric-card">
          <div className="metric-icon">👁</div>
          <div className="metric-value">{fmtViews(user.views)}</div>
          <div className="metric-label">Total Views</div>
          <div className="metric-sub"><span className="live-dot" style={{marginRight:4}} /> Live</div>
        </div>
        <div className="metric-card">
          <div className="metric-icon">≈</div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>Date</th><th>Title</th><th>Type</th><th>Format</th><th>Views</th><th>Watch</th></tr></thead>
            <tbody>
              {myVideos.length === 0
                ? <tr><td colSpan="6" style={{ textAlign:'center', color:'var(--text3)', padding: 28 }}>Abhi koi video nahi. Entry add karo!</td></tr>
                : myVideos.map(v => (
                  <tr key={v.id}>
                    ...
                    <td><span className="views-cell"><span className="live-dot" />{fmtViews(v.views)}</span></td>
                    <td><a href={v.link} target="_blank" rel="noreferrer" style={{ color:'var(--gold)', fontWeight:700, textDecoration:'none' }}>▶</a></td>
                  </tr>
                ))
              }
            </tbody>
      ...
      <div className="card">
        ...
        <div className="card-body">
          {myActivity.length === 0
            ? <div style={{ color:'var(--text3)', textAlign:'center', padding: 20 }}>Koi activity record nahi</div>
